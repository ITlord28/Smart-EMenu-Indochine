...
class _TableDetailScreenState extends State<TableDetailScreen> {
  ...
  Future<void> _updateTableStatus(String status) async {
    await FirebaseFirestore.instance
      ...
      if (status == 'empty' || status == 'locked' || status == 'booked')
        'entryTime': null,
    });
    if (mounted) Navigator.pop(context);
  }

  Future<void> _checkout(List<OrderModel> orders, List<OrderItem> doneItems,
    ...
    try {
            ...
            'items': doneItems
                .map((item) => {
                      ...
                      'acceptedAt': item.acceptedAt != null
                          ? Timestamp.fromDate(item.acceptedAt!)
                          : null,
                      'chefId': item.chefId,
                    })
                .toList(),
      ...
      for (var order in orders) {
        ...
        final doneInThisOrder = order.orderItems!
            .where((item) => item.status == 'done' || item.status == 'pending')
            .toList();
        final remainingInThisOrder = order.orderItems!
            .where((item) => item.status == 'cooking')
            .toList();
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          final bool isOccupied =
              widget.table.status == 'occupied' || orders.isNotEmpty;

          final List<OrderItem> doneItems =
              []; // Thanh toán được (done + pending)
              ...
              Expanded(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      const Text('Chi tiết món ăn theo trạng thái',
                          style: TextStyle(
                              fontSize: 22,
                      ...
                      if (orders.isEmpty)
                        const Expanded(
                            child: Center(
                                child: Text('Bàn này chưa gọi món nào.')))
                      else
                        Expanded(
                          child: ListView(
                            children: [
                    ...
                    children: [
                      ...
                      if (widget.table.status == 'empty')
                        ElevatedButton.icon(
                          ...
                          style: ElevatedButton.styleFrom(
                            padding: const EdgeInsets.all(16),
                            backgroundColor:
                                isOccupied ? Colors.grey.shade300 : null,
                          ),
                        ),
                      if (widget.table.status == 'empty')
                        const SizedBox(height: 16),
                      ...
                      if (widget.table.status != 'empty')
                        ElevatedButton.icon(
                          onPressed: isOccupied
                              ? null
                              : () => _updateTableStatus('empty'),
                          icon: const Icon(Icons.clear),
                          label: Text(widget.table.status == 'locked'
                              ? 'Mở khóa / Trống'
                              : 'Hủy bàn / Trống'),
                          style: ElevatedButton.styleFrom(
                            padding: const EdgeInsets.all(16),
                            backgroundColor:
                                isOccupied ? Colors.grey.shade300 : Colors.grey,
                          ),
                        ),
                      ...
                      // Checkout button
                      ElevatedButton.icon(
                        onPressed: doneItems.isEmpty || _isLoading
                            ? null
                            : () => _checkout(orders, doneItems, grandTotal),
                        icon: _isLoading
