...
class _CashierHistoryScreenState extends State<CashierHistoryScreen> {
  ...
  Future<void> _deleteInvoice(String docId) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
            'Hành động này sẽ xóa hoàn toàn hóa đơn khỏi cơ sở dữ liệu. Bạn có chắc chắn muốn hủy hóa đơn này?'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(context, false),
              child:
                  const Text('Quay lại', style: TextStyle(color: Colors.grey))),
          ElevatedButton(
  ...
  Widget build(BuildContext context) {
              ...
              Row(
                children: [
                  ...
                  ChoiceChip(
                    ...
                    selectedColor: AppColors.primary,
                    labelStyle: TextStyle(
                        color: _filterMode == FilterMode.table
                            ? Colors.white
                            : Colors.black,
                        fontWeight: FontWeight.bold),
                    onSelected: (val) {
                  ...
                  ChoiceChip(
                    ...
                    selectedColor: AppColors.primary,
                    labelStyle: TextStyle(
                        color: _filterMode == FilterMode.day
                            ? Colors.white
                            : Colors.black,
                        fontWeight: FontWeight.bold),
                    onSelected: (val) {
                  ...
                  ChoiceChip(
                    ...
                    selectedColor: AppColors.primary,
                    labelStyle: TextStyle(
                        color: _filterMode == FilterMode.month
                            ? Colors.white
                            : Colors.black,
                        fontWeight: FontWeight.bold),
                    onSelected: (val) {
                ...
                Row(
                  children: [
                    OutlinedButton.icon(
                      ...
                      label: Text(
                          'Chọn ngày: ${DateFormat('dd/MM/yyyy').format(_dayFilter)}',
            ...
            builder: (context, snapshot) {
                ...
                return const Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 16),
                      Text('Không tìm thấy hóa đơn nào trong database.',
              ...
              if (_searchKeyword.isNotEmpty) {
                ...
                docs = docs.where((doc) {
                  final data = doc.data() as Map<String, dynamic>;
                  final tableId =
                      (data['tableId'] ?? '').toString().toLowerCase();
                  ...
                  for (var orderMap in ordersList) {
                    ...
                    for (var itemMap in itemsList) {
                      final String name =
                          (itemMap['name'] ?? '').toString().toLowerCase();
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Trung Tâm Quản Lý Hóa Đơn & Lịch Sử Giao Dịch',
            style: TextStyle(
